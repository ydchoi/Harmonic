...
\eQb
(S_N f)^{'} &=& \sum_{|n| \leq N} 2\pi i n \hat{f}(n)e(n),
\eQe  
and
\eQb
\left| \sum_{N \geq |n| > M}2\pi i n \hat{f}(n) e(n) \right| &\leq& 
\sum_{N \geq |n| > M} 2\pi|n||\hat{f}(n)|,
\eQe
for any $M < N$.
...
\eQb
F(r) &=& \sum_{n \in \mathbb{Z}} \hat{f}(n) r^{n},
\eQe
...
\left\{
    \begin{array}{ll}
        0  & \mbox{if } |n| < 2 \\
        \dfrac{1}{2 i n \log(|n|)} & \mbox{otherwise}
    \end{array}
\right.
,
\eQe 
...
\left\{
    \begin{array}{ll}
        0  & \mbox{if } |n| < 2 \\
        \dfrac{1}{2 |n| \log(|n|)} & \mbox{otherwise}
    \end{array}
\right. .
\eQe
By the comparison test, we obtain
\eQb
||f_s||_{H^{\frac{1}{2}}(\mathbb{T})} &=& |\hat{f}(0)| + 
\sum_{n \in \mathbb{Z}} |n||\hat{f_s}(n)|^2 \\
&=& \dfrac{1}{2} \sum_{k=2}^{\infty}\dfrac{1}{n\log^2(n)} \leq 
\dfrac{1}{2} \int_{2}^{\infty} \dfrac{1}{x\log^2(x)} dx =  \dfrac{1}{\log(2)}< \infty. 
\eQe 
...
\eQb
\sum_{k=2}^{n} \sin(2\pi k x) &=& \dfrac{\cos(\pi x)\cos(2\pi x)- \cos(n\pi x)\cos((n+1)\pi x)}
{\sin(\pi x)}.
\eQe
...
\eQb
|g_{n,m}| &=& \left| 
\sum_{k=n}^{m} (\dfrac{1}{n\log(n)} - \dfrac{1}{\log(n+1)})\sum_{i=n}^{k}\sin(2\pi n x) \right|
\leq \dfrac{2}{\log(n)} \\ 
\eQe
...
\eQb
\sum_{n=2}^{\infty} \dfrac{\cos(2\pi n x)}{n\log(n)} 
&=& \sum_{n=2}^{m} \dfrac{\cos(2\pi n x)}{n\log(n)}
+ \sum_{n=m+1}^{\infty} \dfrac{\cos(2\pi n x)}{n\log(n)} \\ 
&\geq& \sum_{n=2}^{m} \dfrac{1- 2\pi^2 n^2 x^2}{n\log(n)}
+ \sum_{n=m+1}^{\infty} \dfrac{\cos(2\pi n x)}{n\log(n)} 
\eQe
Observe that for any $p > m + 1$, we have
\eQb
\left| \sum_{n=m+1}^{p} \dfrac{\cos(2\pi n x)}{n\log(n)} \right|
&=&| \sum_{n=m+1}^{p-1} (\dfrac{1}{n\log(n)} - \dfrac{1}{(n+1)\log(n+1)})
\sum_{k=m+1}^{n} \cos(2\pi k x) \\
&+& \dfrac{1}{p\log(p)} \sum_{k=m+1}^{p}\cos(2\pi k x) | \leq \dfrac{4}{\log(m)}. 
\eQe
Hence, for $m$ large enough, we have
\eQb
f_c(x) &\leq& \int_{2}^{m} \dfrac{1}{x\log(x)} dx - \dfrac{4}{\log(m)} - C \\
&\leq& C''\log\log(m) \leq C'''\log(\log(\frac{1}{|x|})),
\eQe
...
&\leq& 2\pi \sum_{m=1}^{K} (3^{m - K})^{1-\alpha} + 2\dfrac{1}{1-3^{-\alpha}}  \\ 
&=& 
2\pi(3^{1-K})^{1-\alpha}\dfrac{3^{K(1-\alpha)} - 1}{3^{1-\alpha} -1} + \dfrac{2}{1-3^{-\alpha}}\\ 
&\leq& 2\pi \dfrac{3^{1-\alpha}}{3^{1-\alpha} - 1} + \dfrac{2}{1-3^{-\alpha}}. \\ 
\eQe
...
\eQb
\hat{F}(\pm 3^m) = \dfrac{1}{2\cdot 3^{m\alpha}}.
\eQe
...
\eQb
||f||_{H^{\beta}(\mathbb{T})}^2 &=& \sum_{k \in \mathbb{N}} |2^k|^{2\beta}|2^{-\alpha k}|^2 \\
&=& \sum_{k \in \mathbb{N}} 2^{2k(\beta - \alpha)} = \infty ,\\ 
\eQe
