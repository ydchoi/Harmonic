...
the integration result for $f \in S$), substituting $\hat{f}$
...
the inversion formula to $\hat{f} = \hat{f}X$ gives
\eQb
f &=& f * \check{X}. 
\eQe
Since $X \in S$, it follows that $\check{X} \in S$, so $\check{X} \in L^p$ for any $p \geq 1$.
Therefore for any $p \geq 1$, by Young's inequality, we obtain
\eQb
||f||_{p} \leq ||f||_{1} ||\check{X}||_{p} < \infty,
\eQe
as required. 
...
\textbf{(b)} 
Let $f \in L^1$. It follows that $\hat{f'} = -2\pi i \hat{f}$. Define $g$ by
\eQb
g(x) &=& -2\pi i \int \hat{f}(\xi) e^{ix\xi}d\xi. 
\eQe
...
$\hat{f}(\xi)e^{2\pi i \xi z}$ is continuous, so $(i)$ is satisfied. $F$ 
...
$\{\hat{f_n}\}$ converges to $\hat{f}$ in $L^2$. Since the compactness of the support 
...
let $\eta_{h}$ be defined by $(\eta_{h}f)(x) = f(x-h)$ on $L^2$. If $g \in B_{\Lambda}^2$, 
